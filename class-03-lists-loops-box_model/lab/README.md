...
- To improve the user experience, consider whether you should ask for the user's name and then use it in the guessing game, and also if an alert window with instructions to the guessing game would be useful.
- For the first five yes/no questions, try to implement a `while` such that users who have entered an invalid answer (such as a number or "potato") get ongoing opportunities to enter a valid answer.
...
- Do not include any functions in your JavaScript.
- Expect that you might will probably could maybe need both 'for' and 'while' structures for your sixth and seventh questions. But not necessarily.
- Git processes: Do regular commits! As you finish an individual thing, ACP it! For todays assignment, you should *as a minimum* have a separate commit in place for the when you have finished each of the sixth and seventh questions, plus one for the edits within the HTML document.
